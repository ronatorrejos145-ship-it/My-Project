...
### 2. Product Catalog & Service Package Versioning (Phase 4)
...
- `service_package_versions`: Historical price and speed versioning table. Protects old invoices and active subscriber contracts from historical pricing corruption when plan prices are revised. Stores `version_number`, `effective_from`, `effective_until`, `price`, `installation_fee`, `change_reason`, and approval records.
- `package_features` & `package_feature_service_package`: Reusable feature tags (`BOOLEAN`, `TEXT`, `NUMBER`, `AMOUNT`, `PERCENTAGE`, `LIMIT`).
- `promotions` & `promotion_service_package`: Campaign promotions (`FREE_INSTALLATION`, `DISCOUNT`, `FIRST_MONTH_FREE`) with start/end dates, usage limits, and stackability rules.
...
### 3. Customer Management & CRM 360 (Phase 3)
...
### 6. Hardware Assets & Technical Tools
...
- `discounts`: Promotional discount codes and feature matrix.
