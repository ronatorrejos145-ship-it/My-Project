...
@section('header', 'Edit Department: ' . $department->name)
...
<div class="max-w-2xl bg-white p-6 rounded shadow-sm border border-gray-200">
    <form method="POST" action="{{ route('admin.departments.update', $department) }}" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $department->name) }}" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded shadow-sm">
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700">Description</label>
            <textarea name="description" rows="2" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded shadow-sm">{{ old('description', $department->description) }}</textarea>
        </div>
        ...
        <div class="flex justify-end gap-2 pt-4">
            ...
            <button type="submit" class="px-4 py-2 bg-sky-600 text-white rounded text-sm font-medium">Update Department</button>
        </div>
    </form>
</div>
@endsection
